...
void EllipHavi(double *pointX, double *pointY, int amountPoint, double &bigAxis, double &littleAxis, bool ModuleTest) {
	int i, j, indexMaxPointOne, indexMaxPointTwo, centerX, centerY;
	int distance;
	double tiltAngel = 0.0;
	for (i = 0; i < amountPoint; i++) {

		for (j = 1; j < amountPoint; j++) {
				...
				if (!fin.is_open()) {
					do {
						...
						case (creatFile): {
							...
							PrintToFile(fout, initialPoint, amountPoint, bigAxis, littleAxis);
							fout.close();
...
void PrintToFile(ofstream &fout, double *initialPoint, const int amountPoint, double bigAxis, double  littleAxis, bool initial) {
	if (initial) {
		fout << amountPoint << " ";
		for (int i = 0; i < amountPoint * 2; i++) {
			fout << initialPoint[i] << " ";
		}
	}
			...
			do {
				...
				if (!fin.is_open()) {
					do {
						...
						case (creatFile): {
							ofstream fout(fileName); //создать
							PrintToFile(fout, initialPoint, amountPoint, 0, 0, true);
							fout.close();
							cout << "Данные успешно сохранены." << endl;
							system("pause");
							return 0;
