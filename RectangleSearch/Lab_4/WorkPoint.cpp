				...
				if (!fin.is_open()) {
					do {
						...
						case (creatFile): {
							...
							PrintToFile(fout, initialPoint, amountPoint, P, S);
							fout.close();
...
void PrintToFile(ofstream &fout, double *initialPoint, const int amountPoint, double P, double S, bool initial) {
	if (initial) {
		fout << amountPoint << " ";
...
void convexHull(Point *points, int n)
{
	...
	do {
		switch (userSelect) {
		case 1: {
			RectangleWork(pointX, pointY, count, P, SS);
			select = false;
			break;
		}

		case 2: {
			SquareWork(pointX, pointY, count, P, SS);
			select = false;
			break;
		}
		default: {
